...
const Search = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      dispatch({
          type: GLOBALTYPES.ALERT, payload: {error: err.response.msg}
      })
    }
  }
  ...
  return (
    <form className='search_form' onSubmit={handleSearch}>
        ...
        <div className="search_icon d-flex align-items-center" style={{opacity: search ? 0 : 0.3}}>
            <span className="material-symbols-outlined">search</span>
            <span className='ms-1'>Enter To Search</span>
        </div>
