...
const authCtrl = {
    register: async (req , res) => {
        try {
            ...
            const newUser = new Users({
                fullname , username:newUserName , email:newEmail , password:passwordHash , gender
            })
    ...
    login: async (req , res) => {
        try {
            ...
            if(!user)return res.status(400).json({ msg:"This email is incorrect."})
    ...
    generateAccessToken: async (req , res) => {
        try {
            ...
            if(!rf_token) return res.status(400).json({ msg:"you are not logged in."})

            jwt.verify(rf_token, process.env.REFRESH_TOKEN_SECRET , async(err , result)=>{
                if(err) return res.status(400).json({ msg:"you are not logged in."})
